...
// static __global__ void computeRHSAndUpdateSV(const int Nx, const int Ny, const real delta_t, const real phi_x, const real phi_y, const real phi_xy,
//                                              const real diff_coeff_x, const real diff_coeff_y, const real diff_coeff_xy, const real actualTime, const int num_active_stimuli,
...
//         real diff_term = compute_diffusion_term_anisotropic(d_Vm, i, j, Nx, Ny, diff_coeff_x, diff_coeff_y, diff_coeff_xy, phi_x, phi_y, phi_xy);
//         real Vmtilde = actualVm + 0.5f * diff_term + (0.5f * delta_t * (stim - select_compute_dVmdt(cell_model, actualVm, d_actualsV)));

//         // Calculate approximation for state variables and update them
...
//         // Calculate fully and explicitly the RHS
...
//                                     const real *d_Vm, const real *d_RHS, real *d_RHS)
// {
//     // Obtain the thread index
//     const int i = blockIdx.y * blockDim.y + threadIdx.y;
//     const int j = blockIdx.x * blockDim.x + threadIdx.x;
...
//         d_RHS[idx] = Vm_center + diff_term + 0.5f * d_RHS[idx];
//     }
// }
...
//     const real sigma_i = config->sigma_i;
...
//     const real phi_xy = delta_t / (2.0f * delta_x * delta_y);
//     const real diff_coeff_x = compute_diffusion_coefficient(sigma_l);
//     const real diff_coeff_y = compute_diffusion_coefficient(sigma_t);
//     const real diff_coeff_xy = compute_diffusion_coefficient(sigma_i);
...
//     populateDiagonalThomasAlgorithm(la_x, lb_x, lc_x, Nx, tau * phi_x * diff_coeff_x);
//     populateDiagonalThomasAlgorithm(la_y, lb_y, lc_y, Ny, tau * phi_y * diff_coeff_y);
...
//         // =================================================
//         //  Calculate first step RHS and Update ODEs     
//         // =================================================
//         startTime = omp_get_wtime();

//         // Launch kernel to compute the reaction term and update state variables
//         computeRHSAndUpdateSV<<<fullDomainGridSize, fullDomainBlockSize>>>(Nx, Ny, delta_t, phi_x, phi_y, diff_coeff, actualTime,
...
//         prepareRHS_x<<<fullDomainGridSize, fullDomainBlockSize>>>(Nx, Ny, phi_x * diff_coeff * tau, d_Vm, d_RHS, d_RHS);
//         CUDA_CALL(cudaDeviceSynchronize());
...
//         // =================================================
//         prepareRHS_y<<<fullDomainGridSize, fullDomainBlockSize>>>(Nx, Ny, phi_y * diff_coeff * tau, d_RHS, d_RHS, d_Vm);
//         CUDA_CALL(cudaDeviceSynchronize());
