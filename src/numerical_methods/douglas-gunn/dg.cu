...
static __global__ void computeReactionApproxAndUpdateSV(const int Nx, const int Ny, const real delta_t, const real delta_x, const real delta_y,
                                                        ...
                                                        const real *d_Vm, real *d_sV, real *d_reaction, const real *d_Dxx, const real *d_Dyy, const real *d_Dxy, const CellModel cell_model, const real denom_chiCm, const bool is_aligned)
{
    ...
    if (i < Ny && j < Nx)
    {
        ...
        // Calculate aproximation with RK2 -> Vm* = Vmn + 0.5*dt*(diffusion + R(Vmn, sVn))
        const real diff_term = select_compute_diffusion_term(is_aligned, d_Vm, d_Dxx, d_Dyy, d_Dxy, i, j, Nx, Ny, delta_x, delta_y);

        const real Vmtilde = actualVm_center + 0.5f * delta_t * ((diff_term * denom_chiCm) + stim - select_compute_dVmdt(cell_model, actualVm_center, d_actualsV));
...
static __global__ void prepareRHS_for_ximplicit(const int Nx, const int Ny, const real delta_x, const real delta_y, const real delta_t, const real *d_Vm,
                                                const real *d_reaction, real *d_RHS, const real *d_Dxx, const real *d_Dyy, const real *d_Dxy, const real denom_chiCm, const bool is_aligned)
{
    // Obtain the thread index
    const int i = blockIdx.y * blockDim.y + threadIdx.y;
    const int j = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (i < Ny && j < Nx)
    {
        // Calculate the RHS of the linear system with explicit diffusion term along x and y
        const real Vm_center = d_Vm[idx];
        const real diff_term_x = select_compute_diffusion_term_x(is_aligned, d_Vm, d_Dxx, d_Dxy, i, j, Nx, Ny, delta_x, delta_y);
        const real diff_term_y = select_compute_diffusion_term_y(is_aligned, d_Vm, d_Dyy, d_Dxy, i, j, Nx, Ny, delta_x, delta_y);
        d_RHS[idx] = Vm_center + delta_t * ((0.5f * diff_term_x * denom_chiCm) + (1.0f * diff_term_y * denom_chiCm) + d_reaction[idx]);
    }
}

static __global__ void prepareRHS_for_yimplicit(const int Nx, const int Ny, const real delta_x, const real delta_y, const real delta_t, const real *d_Vm,
                                                const real *d_Vmhalf, real *d_RHS, const real *d_Dyy, const real *d_Dxy, const real denom_chiCm, const bool is_aligned)
{
    // Obtain the thread index
    const int i = blockIdx.y * blockDim.y + threadIdx.y;
    const int j = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (i < Ny && j < Nx)
    {
        ...
        const real Vmhalf_center = d_Vmhalf[idx];
        const real diff_term_y = select_compute_diffusion_term_y(is_aligned, d_Vm, d_Dyy, d_Dxy, i, j, Nx, Ny, delta_x, delta_y);
        d_RHS[idx] = Vmhalf_center - 0.5f * delta_t * (diff_term_y * denom_chiCm);
    }
}
...
void runDG_CUDA(const SimulationConfig *config, Measurement *measurement, const real *time_array, const CellModelSolver *cell_model_solver, real *Vm, real *sV, const real *Dxx, const real *Dyy, const real *Dxy)
{
    ...
    real *d_reaction, *d_RHS;
    CUDA_CALL(cudaMalloc(&d_reaction, total_points * sizeof(real)));
    ...
    SIMPLEMSG("");
    INFOMSG("Starting simulation with DG (CUDA)...\n");
    ...
    while (timeStepCounter < M)
    {
        ...
        computeReactionApproxAndUpdateSV<<<fullDomainGridSize, fullDomainBlockSize>>>(Nx, Ny, delta_t, delta_x, delta_y, actualTime, numberOfStimuli, d_stimuli,
        ...
        //  diffusion implicit in x and explicit in x and y
        // =================================================
        startTime = omp_get_wtime();

        prepareRHS_for_ximplicit<<<fullDomainGridSize, fullDomainBlockSize>>>(Nx, Ny, delta_x, delta_y, delta_t, d_Vm, d_reaction, d_RHS, d_Dxx, d_Dyy, d_Dxy, denom_chiCm, is_aligned);
        CUDA_CALL(cudaDeviceSynchronize());
        ...
        // =================================================
        //  Second step -> Result goes to d_reaction
        //  diffusion implicit in y and explicit in y
        // =================================================
        prepareRHS_for_yimplicit<<<fullDomainGridSize, fullDomainBlockSize>>>(Nx, Ny, delta_x, delta_y, delta_t, d_Vm, d_RHS, d_reaction, d_Dyy, d_Dxy, denom_chiCm, is_aligned);
        CUDA_CALL(cudaDeviceSynchronize());

        startLSTime = omp_get_wtime();

        parallelThomas_y<<<gridSize_x, THOMAS_KERNEL_BLOCK_SIZE>>>(Nx, Ny, d_reaction, thomas_coeff_y, d_Dyy);
        CUDA_CALL(cudaDeviceSynchronize());

        elapsedTime2ndLS += omp_get_wtime() - startLSTime;

        // Copy the result to d_Vm
        CUDA_CALL(cudaMemcpy(d_Vm, d_reaction, total_points * sizeof(real), cudaMemcpyDeviceToDevice));
