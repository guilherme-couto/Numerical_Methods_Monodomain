...
static __global__ void computeYDiffusionReactionApproxAndUpdateSV(const int Nx, const int Ny, const real delta_t, const real delta_x, const real delta_y,
                                                            const real actualTime, const int num_active_stimuli, const Stimulus *d_active_stimuli,
                                                            const real *d_Vm, real *d_sV, real *d_reaction, real *d_auxVm, const real *d_Dxx, const real *d_Dyy, const real *d_Dxy, const CellModel cell_model, const real denom_chiCm, const bool is_aligned)
{
    ...
    if (i < Ny && j < Nx)
    {
        ...
        // Calculate diffusion explicitly in y and save the result in d_auxVm
        d_auxVm[idx] = actualVm_center + 0.5f * delta_t * (select_compute_diffusion_term_y(is_aligned, d_Vm, d_Dyy, d_Dxy, i, j, Nx, Ny, delta_x, delta_y) * denom_chiCm);
...
static __global__ void prepareRHS_for_ximplicit(const int Nx, const int Ny, const real delta_x, const real delta_y, const real delta_t, const real *d_Vm,
                                                const real *d_reaction, const real *d_auxVm, real *d_RHS, const real *d_Dxx, const real *d_Dxy, const real denom_chiCm, const bool is_aligned)
{
    // Obtain the thread index
    const int i = blockIdx.y * blockDim.y + threadIdx.y;
    const int j = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (i < Ny && j < Nx)
    {
        ...
        const real auxVm_center = d_auxVm[idx];
        const real diff_term_x = select_compute_diffusion_term_x(is_aligned, d_auxVm, d_Dxx, d_Dxy, i, j, Nx, Ny, delta_x, delta_y);
        d_RHS[idx] = auxVm_center + delta_t * ((0.5f * diff_term_x * denom_chiCm) + d_reaction[idx]);
    }
}
...
void runDY_CUDA(const SimulationConfig *config, Measurement *measurement, const real *time_array, const CellModelSolver *cell_model_solver, real *Vm, real *sV, const real *Dxx, const real *Dyy, const real *Dxy)
{
    ...
    SIMPLEMSG("");
    INFOMSG("Starting simulation with D'Yakonov (CUDA)...\n");
    ...
    while (timeStepCounter < M)
    {
        ...
        computeYDiffusionReactionApproxAndUpdateSV<<<fullDomainGridSize, fullDomainBlockSize>>>(Nx, Ny, delta_t, delta_x, delta_y, actualTime, numberOfStimuli, d_stimuli,
                                                                                      d_Vm, d_sV, d_reaction, d_auxVm, d_Dxx, d_Dyy, d_Dxy, cell_model, denom_chiCm, is_aligned);
        ...
        prepareRHS_for_ximplicit<<<fullDomainGridSize, fullDomainBlockSize>>>(Nx, Ny, delta_x, delta_y, delta_t, d_Vm, d_reaction, d_auxVm, d_RHS, d_Dxx, d_Dxy, denom_chiCm, is_aligned);
        CUDA_CALL(cudaDeviceSynchronize());
        ...
        // =================================================
        //  Second step -> Result goes to d_RHS
        //  diffusion implicit in y and explicit in y
        // =================================================
        startLSTime = omp_get_wtime();

        parallelThomas_y<<<gridSize_x, THOMAS_KERNEL_BLOCK_SIZE>>>(Nx, Ny, d_RHS, thomas_coeff_y, d_Dyy);
        CUDA_CALL(cudaDeviceSynchronize());
