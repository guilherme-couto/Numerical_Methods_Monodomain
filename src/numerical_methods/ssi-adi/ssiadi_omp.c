...
void runSSIADI_OMP(const SimulationConfig *config, Measurement *measurement, const real *time_array, const CellModelSolver *cell_model_solver, real *Vm, real *sV, const real *Dxx, const real *Dyy, const real *Dxy)
{
    ...
    const int number_of_threads = config->number_of_threads;
    ...
    SIMPLEMSG("");
    INFOMSG("Starting simulation with SSIADI (OpenMP with %d threads)...\n", number_of_threads);

    omp_set_num_threads(number_of_threads);
    #pragma omp parallel default(shared) private(i, j, idx, diff_term, stim, actualVm, Vmtilde, actualsV, sVtilde)
    {
        // Auxiliary arrays for Thomas algorithm
        real *local_c_prime, *local_d_prime, *local_ls_rhs, *local_result;
        if (Nx > Ny)
        {
            local_c_prime = (real *)malloc(Nx * sizeof(real));
            local_d_prime = (real *)malloc(Nx * sizeof(real));
            local_ls_rhs = (real *)malloc(Nx * sizeof(real));
            local_result = (real *)malloc(Nx * sizeof(real));
        }
        else
        {
            local_c_prime = (real *)malloc(Ny * sizeof(real));
            local_d_prime = (real *)malloc(Ny * sizeof(real));
            local_ls_rhs = (real *)malloc(Ny * sizeof(real));
            local_result = (real *)malloc(Ny * sizeof(real));
        }
        ...
        while (timeStepCounter < M)
        {
            ...
            for (j = 0; j < Nx; j++)
            {
                ...
                for (i = 0; i < Ny; i++)
                {
                    ...
                    local_ls_rhs[i] = actualVm + 0.5f * delta_t * ((diff_term * denom_chiCm) + reaction[idx]);
                }

                tridiagonalSystemSolver_y(Ny, local_ls_rhs, local_result, local_c_prime, local_d_prime, thomas_coeff_y, Dyy, j, Nx);

                // Update with the result
                for (i = 0; i < Ny; i++)
                {
                    idx = i * Nx + j;
                    auxVm[idx] = local_result[i];
                }
            }
            #pragma omp barrier
            ...
            for (i = 0; i < Ny; i++)
            {
                ...
                for (j = 0; j < Nx; j++)
                {
                    ...
                    local_ls_rhs[j] = actualVm + 0.5f * delta_t * ((diff_term * denom_chiCm) + reaction[idx]);
                }

                tridiagonalSystemSolver_x(Nx, local_ls_rhs, local_result, local_c_prime, local_d_prime, thomas_coeff_x, Dxx, i);

                // Update with the result
                for (j = 0; j < Nx; j++)
                {
                    idx = i * Nx + j;
                    Vm[idx] = local_result[j];
                }
            }
            #pragma omp barrier
