                                            ...
                                            const real *d_Vm, real *d_sV, real *d_Y0, const real *d_Dxx, const real *d_Dyy, const real *d_Dxy, const CellModel cell_model, const real denom_chiCm, const bool is_aligned)
{
    ...
    if (i < Ny && j < Nx)
    {
        ...
        d_Y0[idx] = actualVm_center + delta_t * ((diff_term * denom_chiCm) + stim - dVmdt);
    }
}

static __global__ void computeY0tilde(const int Nx, const int Ny, const real delta_t, const real delta_x, const real delta_y,
                                      const real actualTime, const int num_active_stimuli, const Stimulus *d_active_stimuli,
                                      const real *d_Y, real *d_Y0, real *d_U, const real *d_Dxx, const real *d_Dyy, const real *d_Dxy, const CellModel cell_model, const real denom_chiCm, const bool is_aligned)
{
    // Obtain the thread index
    const int i = blockIdx.y * blockDim.y + threadIdx.y;
    const int j = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (i < Ny && j < Nx)
    {
        // Calculate the explicit part of the RHS, F0 (only cross-derivative term) with Y_k and U_n-1
        const real compute_F0_tn_Yk = compute_diffusion_term_xy(is_aligned, d_Y, d_Dxx, d_Dyy, d_Dxy, i, j, Nx, Ny, delta_x, delta_y) * denom_chiCm;
        const real compute_F0_tnm1_Unm1 = compute_diffusion_term_xy(is_aligned, d_U, d_Dxx, d_Dyy, d_Dxy, i, j, Nx, Ny, delta_x, delta_y) * denom_chiCm;

        // Update d_Y0tilde with the new approximation -> Y_0tilde = Y0 + 0.5 * dt * (F0(t_n, Y_k) - F0(t_n-1, U_n-1))
        d_Y0[idx] =  d_Y0[idx] + 0.5f * delta_t * (compute_F0_tn_Yk - compute_F0_tnm1_Unm1);
    }
}
                                    ...
                                    real *d_RHS, const real *d_Dxx, const real *d_Dxy, const real denom_chiCm, const bool is_aligned)
{
    ...
    if (i < Ny && j < Nx)
    {
        ...
        d_RHS[idx] = prevY_center - CS_THETA * delta_t * diff_term * denom_chiCm;
    }
}
...
void runCS_CUDA(const SimulationConfig *config, Measurement *measurement, const real *time_array, const CellModelSolver *cell_model_solver, real *Vm, real *sV, const real *Dxx, const real *Dyy, const real *Dxy)
{
    ...
    const real thomas_coeff_x = (CS_THETA * delta_t * denom_chiCm) / (delta_x * delta_x);
    const real thomas_coeff_y = (CS_THETA * delta_t * denom_chiCm) / (delta_y * delta_y);
    ...
    SIMPLEMSG("");
    INFOMSG("Starting simulation with CS (CUDA)...\n");
    ...
    while (timeStepCounter < M)
    {
        ...
        computeY0AndUpdateSV<<<fullDomainGridSize, fullDomainBlockSize>>>(Nx, Ny, delta_t, delta_x, delta_y, actualTime, numberOfStimuli, d_stimuli,
                                                                          d_Vm, d_sV, d_Y0, d_Dxx, d_Dyy, d_Dxy, cell_model, denom_chiCm, is_aligned);
        ...
        prepareRHS_x<<<fullDomainGridSize, fullDomainBlockSize>>>(Nx, Ny, delta_x, delta_y, delta_t, d_Vm, d_Y0, d_RHS, d_Dxx, d_Dxy, denom_chiCm, is_aligned);
        CUDA_CALL(cudaDeviceSynchronize());
        ...
        computeY0tilde<<<fullDomainGridSize, fullDomainBlockSize>>>(Nx, Ny, delta_t, delta_x, delta_y, actualTime, numberOfStimuli, d_stimuli,
                                                                    d_Y, d_Y0, d_Vm, d_Dxx, d_Dyy, d_Dxy, cell_model, denom_chiCm, is_aligned);
        CUDA_CALL(cudaDeviceSynchronize());
        ...
        // =================================================
        //  Fourth step -> Result goes to d_Y
        //  diffusion implicit and explicit in y
        // =================================================
        prepareRHS_y<<<fullDomainGridSize, fullDomainBlockSize>>>(Nx, Ny, delta_x, delta_y, delta_t, d_Vm, d_RHS, d_Y, d_Dyy, d_Dxy, denom_chiCm, is_aligned);
        CUDA_CALL(cudaDeviceSynchronize());
        ...
        CUDA_CALL(cudaMemcpy(d_Vm, d_Y, total_points * sizeof(real), cudaMemcpyDeviceToDevice));
